...
const EditTemplate = ({ fetchUserData, template, user, users, categories, departments, onCancel }) => {
  ...
  // New state for impact assessment
...
useEffect(() => {
  if (template) {
    ...
    const templateImpact = JSON.parse(template.impact || '{}');
    ...
    // Set impact assessment data
    if (templateImpact) {
      setImpactData({
        departmentImpacts: templateImpact.departmentImpacts || [],
        affectedSystems: templateImpact.affectedSystems || [],
        implementationEffort: templateImpact.implementationEffort || 'medium',
        trainingRequired: templateImpact.trainingRequired || false,
        impactNotes: templateImpact.impactNotes || ''
      });
    }
    ...
    form.setFieldsValue({
      ...
      lockBorders: templateStructure.locked_elements?.borders || false,
...
const handleSubmit = (status = 'submitted', isAutoSave = false) => {
  ...
  const saveTemplateData = (status, isAutoSave = false) => {
    ...
    // Update a completely structured object with all expected data
    const templateData = {
      // Basic template info
    ...
    >            
      <div style={{ textAlign: 'center', padding: '40px' }}>
        <TemplateEditor 
          ...
          initialContent={JSON.parse(template?.content || null)}
        />
      </div>
    </Card>
  );
