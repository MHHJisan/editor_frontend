...
const Editor = () => {
  ...
  }, [adjustItemPositions, reconstructHighlight, updateTOC, realignFloatingItems]);
  ...
  useEffect(() => {
    ...
    const loadDocument = async () => {
      ...
      if (docData.template_id !== null) {
        try {
          const templateResponse = await axios.get(`/api/get_template/${docData.template_id}`);
            ...
            >
              ...
              {activeReplyId !== comment.id && (
                <div className="mt-2 d-flex align-items-center gap-2">
                  ...
                  >
                    <i className="bi bi-send" style={{ pointerEvents: 'none' }}></i>
                  </div>
                </div>
              )}

              {comment.status === "deleted" && <p className="text-danger mt-2">✖ Deleted</p>}
            </div>
          ))}
        ...
        {floatingElements
          ...
          .map((suggestededit) => (
            ...
            <button 
              ...
              onClick={(e) => {
                e.stopPropagation();
                deleteFloatingElement(suggestededit.id, "suggestededit");
              }}
            >
              <i className="bi bi-trash"></i>
            </button>
